...
BEGIN
    ...
    FOR troncon IN SELECT *
                   ...
                   WHERE trailsection_id != NEW.trailsection_id
                         ...
                         AND GeometryType(ST_Intersection(shape_2d, NEW.shape_2d)) NOT IN ('LINESTRING', 'MULTILINESTRING')
    LOOP
    ...
    --------------------------------------------------------------------
        ...
        IF array_length(intersections_on_new, 1) > 2 THEN
            ...
            LOOP
                ...
                IF coalesce(ST_Length(ST_GeomFromEWKT(segment),true), 0) < 1 THEN
...
*/
            ...
            WITH existing_rec AS (SELECT MAX(et.eventtrailsection_id) AS eventtrailsection_id, e.e_offset, e.shape_2d
                                   ...
                                   WHERE et.evnt = e.event_id
                                     ...
                                     GROUP BY e.event_id, e.e_offset, e.shape_2d
...
CREATE OR REPLACE FUNCTION hike_trailsection_objectid() RETURNS TRIGGER AS $$
BEGIN
  NEW.objectid := NEW.trailsection_id;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS hikster_new_trailsection_objectid_i_trg ON hike_trailsection;
CREATE TRIGGER hikster_new_trailsection_objectid_i_trg BEFORE INSERT ON hike_trailsection FOR EACH ROW EXECUTE PROCEDURE hike_trailsection_objectid();


CREATE OR REPLACE FUNCTION hike_trail_objectid() RETURNS TRIGGER AS $$
BEGIN
  NEW.objectid := NEW.trail_id;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS hikster_new_trail_objectid_i_trg ON hike_trail;
CREATE TRIGGER hikster_new_trail_objectid_i_trg BEFORE INSERT ON hike_trail FOR EACH ROW EXECUTE PROCEDURE hike_trail_objectid();
